...
import ch.qfs.sample.repository.CustomerRepository;
import ch.qfs.sample.repository.search.CustomerSearchRepository;
...
/**
 * REST controller for managing {@link ch.qfs.sample.domain.Customer}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CustomerResource {
    ...
    public ResponseEntity<List<Customer>> searchCustomers(@RequestParam String query, Pageable pageable) {
        ...
        Page<Customer> page = customerSearchRepository.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
